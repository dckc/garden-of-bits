...
- **Worktree manager.** The liaison creates fork worktrees per `WORKTREES.md`, writes both the per-worktree `.garden/worktree.toml` and the journal index entry at `journal/worktrees/<host>/<name>.md`, and decides when to collect. Subagents do not create or destroy worktrees themselves.
