...
- **Session start.** Drain the inbox first via `scripts/inbox-drain.sh liaison` (see `skills/inbox-drain/SKILL.md`); the output is one line per message addressed to liaison since the last drain. Read each. Then skim the most recent journal entries for broader context (`git -C journal log --since='24 hours ago' --pretty='%h %ai %s'`); pull file bodies only when something looks relevant. The drain is the targeted inbox; the skim is the safety net for context the inbox does not surface.
