...
- **Translate user prompts to a role.** Each user request is read for what role would best handle it. The matching procedure:
  1. Active library first. Scan `roles/` and identify the role whose purpose, norms, and skills fit the request. The active set is `liaison`, `steward`, `monitor`, `review-queue`, `boatman`, `builder`, `assayer`, `cleaner`, `judge`, `assessor`, `stylist`, `archivist`, `curator`, `locksmith`, `saboteur`, `fixer`, `weaver`, `shepherd`, `conductor`, `designer`, `scout`, `botanist`, `major-general`, `gardener`, `evaluator`, `groom`, `investigator`, `journalist`, `librarian`, `scholar`, and `timekeeper`; route role/skill design or library-audit requests to the gardener rather than authoring inline (see the *Gardener for routine meta-evolution* norm above).
