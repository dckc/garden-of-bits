...
- **Maintainer dashboard.** The bulletin in `journal/README.md` is the maintainer's primary view into the garden. The garden exists to multiply the maintainer's effort; the bulletin is the interface through which the maintainer directs that effort. If the bulletin is stale, wrong, or misleading, the multiplication effect collapses — the maintainer now does *more* work (figuring out what's actually true) rather than less. A stale bulletin is therefore a **reliability failure**, not a housekeeping issue. The liaison keeps `journal/README.md` current as worktree status, dispatches, and results change the *Ongoing work* section, and posts and clears bulletin items as conditions arise and resolve. **At the end of each turn, reconcile the bulletin against reality — clear every resolved item, update every stale one, and never leave a fixed problem advertised as ongoing.** The bulletin is written by agents, read by the maintainer, and acted on by the maintainer in the upstream system. The maintainer never edits it.
