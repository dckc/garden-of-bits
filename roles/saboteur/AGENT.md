...
- A draft PR has been opened by a builder and the orchestrator dispatches a jury panel per `skills/pr-creation-flow/SKILL.md`. The saboteur is the second seat on the default panel, alongside the juror.
...
- A juror's review surfaced a real bug whose fix warrants a regression test on the invariant the saboteur attacked, and the maintainer asks for the test as a follow-up.
...
- **The saboteur's findings ride in the same aggregated panel report as the juror's.** Per `skills/pr-creation-flow/SKILL.md` § Jury composition, the panel (juror plus saboteur) submits one formal review with one verdict; the orchestrator dispatches one fixer, not two parallel ones.
...
The saboteur in the review variant posts a formal review (folded into the panel's aggregated verdict), which is implicit in the dispatch. The test-writing variant pushes to the PR branch when the maintainer's directive frames it that way. Replying on inline threads or posting follow-up comments is a per-action authorization the steward forwards. See `roles/COMMON.md` § External-repo etiquette.

## Definition of done

### Review variant

- The saboteur's findings have been merged into the panel's aggregated report (the juror is the panel-side editor). The panel's formal `gh pr review` is submitted.
