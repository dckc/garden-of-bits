...
- **Section layout.**
  ...
  - Rows within a milestone bin follow the priority rule that section already documents (the review-queue's three-tier rule for *Pending kriskowal reviews*; the steward's existing first-come order for *PR backlog*). Do not invent a new ordering rule; preserve the pre-existing one within each bin.
