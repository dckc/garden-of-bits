...
- **Quiet on no change.** When the upstream returns 304 Not Modified, do not write a `tick` entry. Just emit a one-line report: `unchanged repo=<repo> last_modified=<ts>`. Bump `last_heartbeat` in the journal index entry only as needed to stay under the reaper's idle threshold (default 1 hour); a steady stream of 304s does not need a heartbeat per minute.
